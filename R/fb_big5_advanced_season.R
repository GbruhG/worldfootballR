...
# Corrected function based on the header analysis (seasons vs seasons_urls)
fb_big5_advanced_season_stats <- function (season_end_year, stat_type, team_or_player, time_pause = 3) 
{
  ...
  if (!stat_type %in% stat_types) 
    stop("check stat type")
  main_url <- "https://fbref.com"
  season_end_year_num <- season_end_year
  
  seasons <- read.csv("https://raw.githubusercontent.com/GbruhG/worldfootballR/refs/heads/main/all_competitions.csv", 
                      stringsAsFactors = F)
  ...
  get_each_big5_stats_type <- function(season_url, time_pause = time_wait) {
    ...
    if(exists("pb") && inherits(pb, "progress_bar")) pb$tick() 
    ...
    if (!exists(".load_page", mode = "function")) {
        stop(".load_page() helper function is missing. You may need to load the full package or define it manually.")
    }
    team_page <- stat_urls %>% .load_page() 
    
    if (team_or_player == "player") {
      stat_df <- team_page %>% rvest::html_nodes(".table_container") %>% 
        rvest::html_nodes("table") %>% rvest::html_table() %>% 
        data.frame()
    } else {
      stat_df <- team_page %>% rvest::html_nodes(".table_container") %>% 
        rvest::html_nodes("table")
      stat_df_for <- stat_df[1] %>% rvest::html_table() %>% 
        data.frame()
      stat_df_against <- stat_df[2] %>% rvest::html_table() %>% 
        data.frame()
    ...
    if (stat_type == "playingtime") {
      new_names <- new_names %>% gsub("\\..[[:digit:]]+", 
                                      "", .) %>% gsub("\\.[[:digit:]]+", "", .) %>% 
        gsub("_Var", "", .) %>% gsub("# Pl", "Num_Players", 
                                     .) %>% gsub("%", "_percent", .) %>% gsub("_Performance", 
                                                                              "", .) %>% gsub("1/3", "Final_Third", .) %>% 
        gsub("/", "_per_", .) %>% gsub("-", "_minus_", 
                                       .) %>% gsub("90s", "Mins_Per_90", .) %>% gsub("\\+", 
                                                                                     "plus", .)
    } else {
      new_names <- new_names %>% gsub("\\..*", "", .) %>% 
        gsub("_Var", "", .) %>% gsub("# Pl", "Num_Players", 
                                     .) %>% gsub("%", "_percent", .) %>% gsub("_Performance", 
                                                                              "", .) %>% gsub("1/3", "Final_Third", .) %>% 
        gsub("/", "_per_", .) %>% gsub("-", "_minus_", 
    ...
    stat_df <- stat_df[-1, ]
    urls <- team_page %>% rvest::html_nodes(".table_container") %>% 
      rvest::html_nodes("table") %>% rvest::html_nodes("tbody") %>% 
      rvest::html_nodes("tr") %>% rvest::html_node("td a") %>% 
      rvest::html_attr("href") %>% paste0(main_url, .)
    if (team_or_player == "player") {
      stat_df <- dplyr::bind_cols(stat_df, Url = urls)
      stat_df <- stat_df %>% dplyr::filter(.data[["Rk"]] != 
                                             "Rk") %>% dplyr::select(-.data[["Rk"]])
      stat_df <- stat_df %>% dplyr::select(-.data[["Matches"]])
      cols_to_transform <- stat_df %>% dplyr::select(-.data[["Squad"]], 
                                                     -.data[["Player"]], -.data[["Nation"]], -.data[["Pos"]], 
                                                     -.data[["Comp"]], -.data[["Age"]], -.data[["Url"]]) %>% 
        names()
      chr_vars_to_transform <- stat_df %>% dplyr::select(.data[["Nation"]], 
                                                         .data[["Comp"]]) %>% names()
    } else {
      stat_df <- stat_df %>% dplyr::filter(.data[["Rk"]] != 
                                             "Rk") %>% dplyr::select(-.data[["Rk"]])
      stat_df <- dplyr::bind_cols(stat_df, Url = urls)
      cols_to_transform <- stat_df %>% dplyr::select(-.data[["Squad"]], 
                                                     -.data[["Comp"]], -.data[["Url"]]) %>% names()
      chr_vars_to_transform <- stat_df %>% dplyr::select(.data[["Comp"]]) %>% 
        names()
    }
    stat_df <- stat_df %>% dplyr::mutate_at(.vars = cols_to_transform, 
                                            .funs = function(x) {
                                              gsub(",", "", x)
                                            }) %>% dplyr::mutate_at(.vars = cols_to_transform, 
                                                                    .funs = function(x) {
                                                                      gsub("+", "", x)
                                                                    }) %>% dplyr::mutate_at(.vars = cols_to_transform, 
                                                                                            .funs = as.numeric) %>% dplyr::mutate_at(.vars = chr_vars_to_transform, 
                                                                                                                                     .funs = function(x) {
                                                                                                                                       gsub("^\\S* ", "", x)
                                                                                                                                     })
    stat_df <- stat_df %>% dplyr::mutate(Team_or_Opponent = ifelse(!stringr::str_detect(.data[["Squad"]], 
                                                                                        "vs "), "team", "opponent")) %>% dplyr::filter(.data[["Team_or_Opponent"]] == 
                                                                                                                                         "team") %>% dplyr::bind_rows(stat_df %>% dplyr::mutate(Team_or_Opponent = ifelse(!stringr::str_detect(.data[["Squad"]], 
                                                                                                                                                                                                                                               "vs "), "team", "opponent")) %>% dplyr::filter(.data[["Team_or_Opponent"]] == 
                                                                                                                                                                                                                                                                                                "opponent")) %>% dplyr::mutate(season_url = season_url, 
                                                                                                                                                                                                                                                                                                                               Squad = gsub("vs ", "", .data[["Squad"]])) %>% dplyr::select(.data[["season_url"]], 
                                                                                                                                                                                                                                                                                                                                                                                                            .data[["Squad"]], .data[["Comp"]], .data[["Team_or_Opponent"]], 
                                                                                                                                                                                                                                                                                                                                                                                                            dplyr::everything())
    stat_df <- seasons %>% dplyr::select(Season_End_Year = .data[["season_end_year"]], 
                                         .data[["seasons"]]) %>% dplyr::left_join(stat_df, 
                                                                                 by = c(seasons = "season_url")) %>% dplyr::select(-.data[["seasons"]]) %>% 
      dplyr::filter(!is.na(.data[["Squad"]])) %>% dplyr::arrange(.data[["Season_End_Year"]], 
                                                                .data[["Squad"]], dplyr::desc(.data[["Team_or_Opponent"]]))
    if (team_or_player == "player") {
  ...
  all_stats_df <- seasons_urls %>% purrr::map_df(get_each_big5_stats_type)
  return(all_stats_df)
}
